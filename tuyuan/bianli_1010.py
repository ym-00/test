...
def bianli_func(img_type:str):
    ...
    files.sort()
    ocr = PaddleOCR(ruse_angle_cls=False, use_gpu=False, lang='en', show_log=False,det_limit_side_len=2500,det_db_box_thresh=0.1, drop_score=0.1)
    ...
    for file in files:
        ...
        for imgpath in imglist:
            ...
            dst1 = cv2.erode(blur_gray, kernel=kernel)
            ret, binary = cv2.threshold(blur_gray, 22, 255, cv2.THRESH_BINARY)
            if count < 10:
            ...
            max_line_gap = 40  # maximum gap in pixels between connectable line segmentsc
            ...
            for i in range(len(list_x)):
                x1, y1, x2, y2 = list_x[i]
                if x1 > x2:
                    list_x[i] = [x2, y2, x1, y1]

            for i in range(len(list_y)):
                x1, y1, x2, y2 = list_y[i]
                if y1 > y2:
                    list_y[i] = [x2, y2, x1, y1]
            list_x=np.array(list_x)
            list_y=np.array(list_y)
            try:
                list_x = list_x[np.lexsort(list_x[:, ::-1].T)]  # 按第一列排序
                list_y = list_y[np.lexsort(list_y[:, ::-1].T)]  # 按第一列排序
            except:
                print("此区域没有线段")
            print(list_x)
            print(len(list_x))
            ...
            for i in range(len(list_y)):
                ...
                for j in range(len(list_x)):
                    x11,y11,x22,y22=list_x[j]
                    distance1=sqrt((x1-x11)**2+(y1-y11)**2)
                    distance2=sqrt((x2-x11)**2+(y2-y11)**2)
                    if distance1<distance_up1:
                        distance_up1 = distance1
                    if distance2<distance_up2:
                        distance_up2 = distance2
                if distance_up1<=30 and distance_up2<=30:
                    flag_y[i]=1
            for i in range(len(list_y)):
                x1,y1,x2,y2=list_y[i]
                distance_up1=999
                ...
                for j in range(len(list_x)):
                    x11,y11,x22,y22=list_x[j]
                    distance1=sqrt((x1-x22)**2+(y1-y22)**2)
            ...
            for i in range(len(list_x)):
                ...
                for j in range(len(list_y)):
                    ...
                    if distance2<distance_up2:
                        distance_up2 = distance2
                if distance_up1<=5 or distance_up2<=5:
                    flag_x[i]=1
            for i in range(len(list_x)):
                x1,y1,x2,y2=list_x[i]
                distance_down1=999
                ...
                for j in range(len(list_y)):
                    ...
                    distance1=sqrt((x1-x22)**2+(y1-y22)**2)
                    distance2=sqrt((x2-x22)**2+(y2-y22)**2)
                    if distance1<distance_down1:
                        distance_down1 = distance1
                    if distance2<distance_down2:
                        distance_down2 = distance2
                if distance_down1<=5 or distance_down2<=5:
                    flag_x[i]=1
            ...
            # for i in list_x:
            #     for j in list_y:
            #         dis_y = abs(i[1] - (j[1] + j[3]) / 2)
            #         dis_x = abs(i[0] - (j[0] + j[2]) / 2)
            #         if dis_y < 10 and dis_x < 10:
            #             print('1')
            #             # i = [0, 0, 0, 0]
            # for i in list_x:
            #     if i[0] == 0 and i[1] == 0:
            ...
            if len(list_x)==0 or len(list_y)==0:
                img_number += 1
                continue
            lines_1 = np.concatenate((list_x, list_y), axis=0)
            print(len(lines_1))
            print(len(lines))
            # exit()
            for line in lines_1:
                ...
                # for i in list1:
                #     if int(x1) == int(i[0]) and int(y1) == int(i[1]):
                #         continue
                #     dis_1 = sqrt((x1 - i[0])**2 + (y1 - i[1])**2)
                #     dis_2 = sqrt((x1 - i[2]) ** 2 + (y1 - i[3]) ** 2)
                #     dis_t = min(dis_1, dis_2)
                #
                #     if dis_t < t:
                #         t = dis_t
                #     x_c = (i[0] + i[2]) / 2
                #     y_c = (i[1] + i[3]) / 2
                #     if abs(x1 - x_c) < 5 and abs(y1 - y_c) < 5:
                #         p = 1
                # for i in list1:
                #     if int(x1) == int(i[0]) and int(y1) == int(i[1]):
                #         continue
                #     dis_3 = sqrt((x2 - i[0]) ** 2 + (y2 - i[1]) ** 2)
                #     dis_4 = sqrt((x2 - i[2]) ** 2 + (y2 - i[3]) ** 2)
                #     dis_t_l = min(dis_3, dis_4)
                #     if dis_t_l < t_l:
                #         t_l = dis_t_l
                #     x_c = (i[0] + i[2]) / 2
                #     y_c = (i[1] + i[3]) / 2
                #     if abs(x2 - x_c) < 5 and abs(y2 - y_c) < 5:
                #         p = 1
                # if t > 15 or t_l > 15:
                #     continue
                # if t < 10 and t_l < 10:
                #     continue
                # print(str(t) + '----' + str(t_l) + '++++' + str(x1) + '_' + str(y1))
                ...
                if 2077 < int(x1) and 2156 < int(x2) < 2191 and 1726 < int(y1) < 1740 and 1726 < int(y2) < 1740:
                    x2 = int(x2) + 170
                    x1 = int(x1) - 10
                if 21 < int(x1) and int(x2) < 27 and 1266 < int(y1) and int(y2) < 1339:
                    y1 = int(y1) + 22
                    cv2.line(line_image, (x1, y1), (x2, y2), (0, 0, 255), 5)
                # if
                else:
            ...
            for c in contours:  # 遍历轮廓
                ...
                if img_number != 6:
                    ...
                    try:
                        ...
                        cv2.rectangle(img_tmp_clean, (x-2, y), (x + w , y + h+2), (255, 255, 255),
                                      -1)  # 在其上【擦除】本次检测到的矩形
