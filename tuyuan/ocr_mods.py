...
def create_descriptors(folder):
    ...
    for f in files:
        if '.jpg' not in f:
            ...
            Note.write('circles ' + file_name + ' ' + f[:-4])
...
def create_descriptors_contours(folder):
    ...
    for f in files:
        if '.jpg' not in f:
            ...
            Note.write('contours ' + file_name + ' ' + f[:-4])
    ...
    for f in files:
        if '.jpg' not in f:
            ...
            for i in range(0,len(result)):
                ...
                img_mask[x_1:x_2-2,y_1+3:y_2-3] = 255
            # cv2.imshow("edges_1", img_mask)
            # cv2.imshow("1", img)
            # cv2.waitKey(0)
            ...
            if len(contours) == 6 and contours[5].shape[0] > 50:
                continue
            if len(contours) == 5 and contours[4].shape[0] > 70:
                continue
            if len(contours) < 5:
                ...
                if contour_num_valid > 2:
                    for c in contours:  # 遍历轮廓
                        ...
                        x_a2 = int(x_a2)
                        y_a2 = int(y_a2)
                        img_cut = origin_img[y_min:y_max + 5, x_min:x_max + 10]
                ...
                # a = list_all.reverse()
                # print(a)
                if not os.path.isdir(path + '/label/'):  # 创建文件夹
                    os.makedirs(path + '/label/')
                for i in list_all:
                    if int(i[0]) == int(len(list_all) - 1):
                        if '/' in i[2]:
                            file_name_ud = i[2].replace('/','_')
                        else:
                            file_name_ud = i[2]
                        Note = open(path + '/label/' + 'contour_' + file_name_ud + '_' + i[1] + '.txt', mode='w')
                        # Note.write(file_name + ' ' + f[:-4])
                list_all.reverse()
                for j in list_all:
                    Note.write('contour ' + j[2] + ' ' + j[1] + '\n')
                cope_file.append(f)
