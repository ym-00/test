...
def contours_2(img, img_number,img_type, x_start, y_start, count):
...
def cir(img, count, imgpath, img_number, img_type,imgname, current_info, filed, img_tmp_mark, img_tmp_clean,
    ...
    filePath = f"./output_{img_type}"
    if count < 10:
        out_path_0 = filePath + f"/output_0%d/circles/" % count
    else:
        out_path_0 = filePath + f"/output_%d/circles/" % count
    out_path = out_path_0 +  str(imgname) + '_circles/'
    if count < 10:
        if not os.path.isdir(filePath + "/output_0%d/circles/" % count):  # 创建文件夹
            os.makedirs(filePath + f"/output_0%d/circles/" % count)
    else:
        if not os.path.isdir(filePath + "/output_%d/circles/" % count):  # 创建文件夹
            os.makedirs(filePath + f"/output_%d/circles/" % count)
    ...
    black = np.copy(img) * 0
    circles = cv2.HoughCircles(edges, cv2.HOUGH_GRADIENT, 1, 50, param1=50, param2=40, minRadius=17, maxRadius=60)
    ...
    for circle in circles[0]:
        ...
        cv2.circle(img, (x0, y0), r0 + 8, (255, 255, 255), -1)  # 2022-11-21新增
