...
from delete import *
from pdf2image_1116 import *


# coding=gbk
# -*- coding: utf-8 -*-
#method: str, paths: str, name: str, pdf_path: str
def main(method: str, paths: str, name: str, pdf_path: str):
    ...
    if len(sys.argv) > 2:      #如果命令行参数大于2
        paths = sys.argv[2]    #路径就是命令行的第二个参数（除去脚本自身）

    if len(sys.argv) > 3:      #如果命令行参数大于3
        name = sys.argv[3]    #json名称就是命令行的第三个参数（除去脚本自身）
        
    if len(sys.argv) > 4:      #如果命令行参数大于4
        pdf_path = sys.argv[4]    #pdf路径就是命令行的第四个参数（除去脚本自身）

    imglist = getFileList(str(paths), [], 'jpg')
